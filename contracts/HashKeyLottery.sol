...
contract HashKeyLottery is Ownable {
  ...
  function isGameStarted() private view returns (bool) {
    ...
    return g.start > 0 && block.timestamp > g.start;
  }

  function isGameEnded() private view returns (bool) {
    require(currentGameId > 0);
    Game storage g = games[currentGameId];
    return g.end > 0 && block.timestamp > g.end;
  }

  function isGameComplete() private view returns (bool) {
    require(currentGameId > 0);
    Game storage g = games[currentGameId];
    return g.complete > 0 && block.timestamp > g.complete;
  }
  ...
  */
   ...
   */
  function newGame(uint _price, uint _feePercent, string _secretKeyHash, uint256 _start, uint256 _end) public onlyOwner returns (uint) {
    ...
    game.count = currentGameId;
    game.price = _price;
    game.feePercent = _feePercent;
    game.secretKeyHash = _secretKeyHash;
    game.start = _start;
    game.end = _end;
  ...
  function play() public payable returns (uint) {
    ...
    uint ticketsPurchased = msg.value / g.price;
  ...
  function getGame(uint gameCount) public view returns (uint, uint, uint, string, uint, uint, uint, bool, bool) {
